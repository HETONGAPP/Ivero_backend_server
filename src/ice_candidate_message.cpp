...
bool IceCandidateMessage::isIceCandidate(const Json::Value &message_json) {
  return WebrtcMessage::isType(message_json, kIceCandidateType);
}

bool IceCandidateMessage::fromJson(const Json::Value &message_json) {
  if (isIceCandidate(message_json)) {
    if (!WebrtcJsonParser::GetStringFromJsonObject(message_json,
                                                   kSdpMidFieldName, &sdp_mid))
      return false;
    if (!WebrtcJsonParser::GetIntFromJsonObject(
...
std::string IceCandidateMessage::toJson() {
  ...
  message_json[WebrtcMessage::kMessageTypeFieldName] = kIceCandidateType;
